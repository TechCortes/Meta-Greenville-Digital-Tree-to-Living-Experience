...
const rarityColor = { Common: '#94a3b8', Rare: '#C6A96B', 'Super Rare': '#e879a0' }
  ...
  return (
    <section id="collection" className={styles.section}>
      ...
      <div ref={gridRef} className={`${styles.grid} reveal`}>
        {nfts.map((n) => (
          <a key={n.id} href={MARKETPLACE} target="_blank" rel="noopener noreferrer" className={styles.card}>
            <div className={styles.thumb} style={{ background: n.g }}>
              <span className={styles.monogram}>MG</span>
            </div>
            <div className={styles.info}>
              <span className={styles.nftId}>No. {n.id}</span>
              <span className={styles.rarity} style={{ color: rarityColor[n.rarity] }}>{n.rarity}</span>
            </div>
          </a>
        ))}
      </div>
      <div className={styles.cta}>
        <a href={MARKETPLACE} target="_blank" rel="noopener noreferrer" className={styles.btn}>Browse the Full Collection</a>
      </div>
    </section>
  )
}
